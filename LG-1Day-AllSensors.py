...
import pandas as pd
import time as tm
dataset = load_files("./Training Data/1Day", load_content=False)
max_epoch = 10
patience = 3
test_count = 6
validation_count = 6
...
model = SGDClassifier(loss="log", penalty="l2", alpha=0.0001, max_iter=3000, tol=None, shuffle=True, verbose=0, learning_rate='adaptive', eta0=0.01, early_stopping=False)
...
for e in range(0, max_epoch):
    ...
    validationLoss = log_loss(validation_data['targets'], validationPredictions)
...
loss = log_loss(test_data['targets'], predictions)
report["model_loss"] = loss
with open("./Model Results/lg/1day/allsensors/results_{:.0f}.json".format(tm.time()), "w") as f:
    f.write(dumps({
